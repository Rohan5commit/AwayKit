...
import type { MatchGroup, SharedExpense } from "@/types"
...
export default function SplitAndSettle() {
  ...
  const handleSettle = async (from: string, to: string, settleAmount: number) => {
    ...
    setSettling(`${from}-${to}`)
    try {
      const address = toMember.walletAddress || ""
      await splitExpense({
  ...
  return (
    <div className="min-h-screen py-6">
      <div className="max-w-2xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        {/* Add Expense */}
        <Button variant="primary" fullWidth onClick={() => setShowAddExpense(true)} icon="➕">
          Add Shared Expense
        </Button>
              ...
              return (
                <div key={exp.id} className="p-3 bg-gray-800/50 rounded-xl border border-gray-700/50">
                  ...
                  <div className="flex items-center justify-between mt-2 pt-2 border-t border-gray-700/50">
                    <span className="text-xs text-gray-500">{exp.perPerson} per person × {exp.splitAmong.length}</span>
        ...
        {/* Add Expense Modal */}
        <Modal isOpen={showAddExpense} onClose={() => setShowAddExpense(false)} title="Add Shared Expense">
          <div className="space-y-4">
            ...
            <Button variant="primary" fullWidth onClick={handleAddExpense} disabled={!description || !amount || selectedMembers.length === 0}>
              Add Expense & Record on WDK
            </Button>
          </div>
        </Modal>
